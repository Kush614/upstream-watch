...
/**
 * Reading a TrueForge event stream.
 *
 * These were the previous UI's adapter. They are now INTERNALS: `adapter.real.ts` is the
 * only thing that imports them, and the UI sees nothing but the `Adapter` interface. They
 * stay because every one of them encodes something learned the hard way from real data.
 *
